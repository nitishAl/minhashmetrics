...
class MinHashApproach(@transient sc:SparkContext) {
  def runLSH(titles : RDD[Set[String]]) = {
    ...
    val vector = titles.map(words => (words.map(word => (WordIdGenerator.getId(word), 1.0)))).map(a => Vectors.sparse(65535, a.toSeq).asInstanceOf[SparseVector])
    val lsh = new  LSH(data = vector, p = 65537, m = 1000, numRows = 1000, numBands = 200, minClusterSize = 2)
    val model = lsh.run
    model
  }
}